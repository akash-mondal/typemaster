...
let AC = null, master = null, musicBus = null, sfxBus = null, musicFilter = null, echoIn = null;
let unlocked = false;
const listeners = [];
...
function makeContext() {
  ...
  const comp = AC.createDynamicsCompressor(); comp.threshold.value = -10; comp.ratio.value = 6; comp.attack.value = 0.004; comp.release.value = 0.2;
  master = AC.createGain(); master.gain.value = 0.8;
  master.connect(dac); dac.connect(comp); comp.connect(AC.destination);
  // music: a filter the game can close (danger, underwater, pause) and the echo send
...
function unlock() {
  ...
  if (!unlocked) { unlocked = true; buildBank(); for (const f of listeners.splice(0)) try { f(); } catch (e) {} }
}
...
function tick() {
  ...
  while (SEQ.nextTime < ahead) {
    ...
    if (SEQ.pending && local % 16 === 0) {
      ...
      if (p.sting) playSting(p.sting, SEQ.nextTime - 60 / SONGS[old.name].bpm);
      if (p.name) {
...
function playSting(name, when) {
  ...
  const out = AC.createGain(); out.gain.value = 1; out.connect(sfxBus);
...
const SFX_DEF = {
  ...
  shuriken_whirr: () => { const d = secs(0.6); for (let i = 0; i < d.length; i++) { const t = i / SR; d[i] = Math.sin(TAU * 2400 * t) * (0.5 + 0.5 * Math.sin(TAU * 38 * t)) * Math.sin(Math.PI * t / 0.6); } return finish(add(d, bandpass(noise(0.6), 3000, 3), 0.4), 10); },
  ...
  gate_boom: () => finish(add(add(shape(osc(1.2, 90, 32, 'sine'), 0.002, 3), gong(2.2), 0.6), shape(onePoleLP(noise(0.3), 600), 0.001, 12), 0.6)),
...
const loops = new Map();
function sfx(name, o) {
  if (!unlocked || !BANK.sfx[name]) return null;
  o = o || {};
  const src = AC.createBufferSource();
  src.buffer = BANK.sfx[name];
  // a little variety so repeats never sound mechanical
  const vary = o.vary == null ? 0.06 : o.vary;
  src.playbackRate.value = (o.pitch || 1) * (1 + (Math.random() * 2 - 1) * vary);
  const g = AC.createGain();
  g.gain.value = o.vol == null ? 0.7 : o.vol;
  let node = g;
  if (o.pan && AC.createStereoPanner) { const p = AC.createStereoPanner(); p.pan.value = Math.max(-1, Math.min(1, o.pan)); g.connect(p); node = p; }
  src.connect(g);
  node.connect(sfxBus);
  src.start(AC.currentTime + (o.delay || 0));
  if (o.duck) duck(o.duck, src.buffer.duration * 0.8);
  return src;
}
function loop(name, o) {
  o = o || {};
  if (!unlocked || !BANK.sfx[name]) return { stop() {}, set() {} };
  const prev = loops.get(name);
  if (prev) { prev.set(o.vol == null ? 0.5 : o.vol); return prev; }
  const src = AC.createBufferSource();
  src.buffer = BANK.sfx[name];
  src.loop = true;
  const g = AC.createGain();
  g.gain.setValueAtTime(0, AC.currentTime);
  g.gain.linearRampToValueAtTime(o.vol == null ? 0.5 : o.vol, AC.currentTime + (o.fade || 0.6));
  src.connect(g); g.connect(sfxBus);
  src.start();
  const h = {
    set(v, r) { g.gain.cancelScheduledValues(AC.currentTime); g.gain.setValueAtTime(g.gain.value, AC.currentTime); g.gain.linearRampToValueAtTime(v, AC.currentTime + (r || 0.3)); },
    stop(fade) { loops.delete(name); g.gain.cancelScheduledValues(AC.currentTime); g.gain.setValueAtTime(g.gain.value, AC.currentTime); g.gain.linearRampToValueAtTime(0, AC.currentTime + (fade || 0.6)); src.stop(AC.currentTime + (fade || 0.6) + 0.05); },
  };
  loops.set(name, h);
  return h;
}
function duck(amount, dur) {
  if (!AC) return;
  const now = AC.currentTime, g = musicBus.gain, base = 0.55 * SEQ.volume;
  g.cancelScheduledValues(now);
  g.setValueAtTime(g.value, now);
  g.linearRampToValueAtTime(base * (1 - amount), now + 0.03);
  g.linearRampToValueAtTime(base, now + 0.03 + dur);
}
...
const KataAudio = {
  ...
  music: {
    ...
    setIntensity(n) { n = Math.max(0, Math.min(3, Math.round(n))); if (n === SEQ.intensity) return; SEQ.intensity = n; if (AC) applyLayers(); },
    ...
    setMuffle(x) { if (!AC) return; musicFilter.frequency.setTargetAtTime(16000 * Math.pow(900 / 16000, Math.max(0, Math.min(1, x))), AC.currentTime, 0.15); },
  ...
  setVolume(masterV, musicV, sfxV) {
    if (!makeContext()) return;
    if (masterV != null) master.gain.value = masterV;
    if (musicV != null) { SEQ.volume = musicV; musicBus.gain.value = 0.55 * musicV; }
    if (sfxV != null) sfxBus.gain.value = 0.8 * sfxV;
  },
  mute(on) { if (makeContext()) master.gain.setTargetAtTime(on ? 0 : 0.8, AC.currentTime, 0.05); },
  // for tooling: render a song's patterns without playing, to check them
  _debug: { SONGS, STINGS, compile, parseNotes, parseHits, get ctx() { return AC; }, get master() { return master; }, BANK },
};
